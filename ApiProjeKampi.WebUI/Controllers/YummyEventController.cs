...
namespace ApiProjeKampi.WebUI.Controllers
{
    public class YummyEventController : Controller
    {
        ...
        public async Task<IActionResult> YummyEventList()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:7000/api/YummyEvents");
        ...
        public async Task<IActionResult> CreateYummyEvent(CreateYummyEventDto createYummyEventDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:7000/api/YummyEvents", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        public async Task<IActionResult> DeleteYummyEvent(int id)
        {
            ...
            await client.DeleteAsync("https://localhost:7000/api/YummyEvents?id=" + id);
        ...
        {
            var client = _httpClientFactory.CreateClient();
            var responseMessage = await client.GetAsync("https://localhost:7000/api/YummyEvents/GetYummyEvent?id=" + id);
            var jsonData = await responseMessage.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateYummyEvent(UpdateYummyEventDto updateYummyEventDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            await client.PutAsync("https://localhost:7000/api/YummyEvents/", stringContent);
            return RedirectToAction("YummyEventList");
        }
    }
}
